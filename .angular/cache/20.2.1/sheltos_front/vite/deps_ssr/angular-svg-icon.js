...
import {
  HttpClient
} from "./chunk-2YFPFDSI.js";
import "./chunk-OZKG6MBX.js";
...
var _SvgIconRegistryService = class _SvgIconRegistryService {
  ...
  loadSvg(url, name = url) {
    ...
    if (this.iconsByUrl.has(name)) {
      return (0, import_rxjs.of)(this.iconsByUrl.get(name));
    } else if (this.iconsLoadingByUrl.has(name)) {
      return this.iconsLoadingByUrl.get(name);
    }
    const o = this.loader.getSvg(url).pipe((0, import_operators.map)((svg) => {
      const div = this.document.createElement("DIV");
      div.innerHTML = svg;
      return div.querySelector("svg");
    }), (0, import_operators.tap)((svg) => this.iconsByUrl.set(name, svg)), (0, import_operators.catchError)((err) => {
      console.error(err);
      return (0, import_rxjs.throwError)(err);
    }), (0, import_operators.finalize)(() => this.iconsLoadingByUrl.delete(name)), (0, import_operators.share)());
  ...
  getSvgByName(name) {
    ...
    return (0, import_rxjs.throwError)(`No svg with name '${name}' has been loaded`);
  }
