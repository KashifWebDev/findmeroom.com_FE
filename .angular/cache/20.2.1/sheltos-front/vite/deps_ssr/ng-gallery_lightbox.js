...
function isDataSource(value) {
  return value && typeof value.connect === "function" && !(value instanceof import_rxjs9.ConnectableObservable);
}
...
var ArrayDataSource = class extends DataSource {
  ...
  connect() {
    return (0, import_rxjs10.isObservable)(this._data) ? this._data : (0, import_rxjs10.of)(this._data);
  }
  disconnect() {
  }
};
...
var FixedSizeVirtualScrollStrategy = class {
  _scrolledIndexChange = new import_rxjs11.Subject();
  /** @docs-private Implemented as part of VirtualScrollStrategy. */
  scrolledIndexChange = this._scrolledIndexChange.pipe((0, import_operators7.distinctUntilChanged)());
...
var ScrollDispatcher = class _ScrollDispatcher {
  ...
  _scrolled = new import_rxjs11.Subject();
  ...
  scrolled(auditTimeInMs = DEFAULT_SCROLL_TIME) {
    if (!this._platform.isBrowser) {
      return (0, import_rxjs11.of)();
    }
    return new import_rxjs11.Observable((observer) => {
      if (!this._cleanupGlobalListener) {
        this._cleanupGlobalListener = this._ngZone.runOutsideAngular(() => this._renderer.listen("document", "scroll", () => this._scrolled.next()));
      }
      const subscription = auditTimeInMs > 0 ? this._scrolled.pipe((0, import_operators7.auditTime)(auditTimeInMs)).subscribe(observer) : this._scrolled.subscribe(observer);
      this._scrolledCount++;
      return () => {
        subscription.unsubscribe();
        this._scrolledCount--;
  ...
  ancestorScrolled(elementOrElementRef, auditTimeInMs) {
    ...
    return this.scrolled(auditTimeInMs).pipe((0, import_operators7.filter)((target) => !target || ancestors.indexOf(target) > -1));
  }
...
var CdkScrollable = class _CdkScrollable {
  ...
  _cleanupScroll;
  _elementScrolled = new import_rxjs11.Subject();
  constructor() {
  }
  ngOnInit() {
...
var ViewportRuler = class _ViewportRuler {
  ...
  _change = new import_rxjs11.Subject();
  /** Used to reference correct document/window */
  _document = inject(DOCUMENT);
  constructor() {
    const ngZone = inject(NgZone);
  ...
  change(throttleTime = DEFAULT_RESIZE_TIME) {
    return throttleTime > 0 ? this._change.pipe((0, import_operators7.auditTime)(throttleTime)) : this._change;
  }
  /** Use defaultView of injected document if available or fallback to global window reference */
  _getWindow() {
    return this._document.defaultView || window;
...
var SCROLL_SCHEDULER = typeof requestAnimationFrame !== "undefined" ? import_rxjs11.animationFrameScheduler : import_rxjs11.asapScheduler;
var CdkVirtualScrollViewport = class _CdkVirtualScrollViewport extends CdkVirtualScrollable {
  ...
  _detachedSubject = new import_rxjs11.Subject();
  /** Emits when the rendered range changes. */
  _renderedRangeSubject = new import_rxjs11.Subject();
  ...
  scrolledIndexChange = new import_rxjs11.Observable((observer) => this._scrollStrategy.scrolledIndexChange.subscribe((index) => Promise.resolve().then(() => this.ngZone.run(() => observer.next(index)))));
  ...
  _viewportChanges = import_rxjs11.Subscription.EMPTY;
  _injector = inject(Injector);
  _isDestroyed = false;
  constructor() {
    super();
  ...
  ngOnInit() {
    ...
    this.ngZone.runOutsideAngular(() => Promise.resolve().then(() => {
      ...
      this.scrollable.elementScrolled().pipe(
        ...
        (0, import_operators7.startWith)(null),
        // Collect multiple events into one until the next animation frame. This way if
        // there are multiple scroll events in the same frame we only need to recheck
        // our layout once.
        (0, import_operators7.auditTime)(0, SCROLL_SCHEDULER),
  ...
  attach(forOf) {
    ...
    this.ngZone.runOutsideAngular(() => {
      this._forOf = forOf;
      this._forOf.dataStream.pipe((0, import_operators7.takeUntil)(this._detachedSubject)).subscribe((data) => {
...
var CdkVirtualForOf = class _CdkVirtualForOf {
  ...
  viewChange = new import_rxjs11.Subject();
  /** Subject that emits when a new DataSource instance is given. */
  _dataSourceChanges = new import_rxjs11.Subject();
  ...
  set cdkVirtualForOf(value) {
    ...
    if (isDataSource(value)) {
      ...
      this._dataSourceChanges.next(new ArrayDataSource((0, import_rxjs11.isObservable)(value) ? value : Array.from(value || [])));
    }
  }
  _cdkVirtualForOf;
  /**
  ...
  dataStream = this._dataSourceChanges.pipe(
    ...
    (0, import_operators7.pairwise)(),
    // Use `_changeDataSource` to disconnect from the previous data source and connect to the
    // new one, passing back a stream of data changes which we run through `switchMap` to give
    // us a data stream that emits the latest data from whatever the current `DataSource` is.
    (0, import_operators7.switchMap)(([prev, cur]) => this._changeDataSource(prev, cur)),
    // Replay the last emitted data when someone subscribes.
    (0, import_operators7.shareReplay)(1)
  );
  ...
  constructor() {
    ...
    this._viewport.renderedRangeStream.pipe((0, import_operators7.takeUntil)(this._destroyed)).subscribe((range) => {
  ...
  _changeDataSource(oldDs, newDs) {
    ...
    this._needsUpdate = true;
    return newDs ? newDs.connect(this) : (0, import_rxjs11.of)();
  }
...
var CloseScrollStrategy = class {
  ...
  enable() {
    ...
    const stream = this._scrollDispatcher.scrolled(0).pipe((0, import_operators8.filter)((scrollable) => {
...
var OverlayRef = class {
  ...
  _injector;
  _renderer;
  _backdropClick = new import_rxjs12.Subject();
  _attachments = new import_rxjs12.Subject();
  _detachments = new import_rxjs12.Subject();
  _positionStrategy;
  _scrollStrategy;
  _locationChanges = import_rxjs12.Subscription.EMPTY;
  _backdropRef = null;
  ...
  _keydownEvents = new import_rxjs12.Subject();
  /** Stream of mouse outside events dispatched to this overlay. */
  _outsidePointerEvents = new import_rxjs12.Subject();
...
var FlexibleConnectedPositionStrategy = class {
  ...
  _positionChanges = new import_rxjs12.Subject();
  /** Subscription to viewport size changes. */
  _resizeSubscription = import_rxjs12.Subscription.EMPTY;
...
var CdkConnectedOverlay = class _CdkConnectedOverlay {
  ...
  _overlayRef;
  _templatePortal;
  _backdropSubscription = import_rxjs12.Subscription.EMPTY;
  _attachSubscription = import_rxjs12.Subscription.EMPTY;
  _detachSubscription = import_rxjs12.Subscription.EMPTY;
  _positionSubscription = import_rxjs12.Subscription.EMPTY;
  _offsetX;
  _offsetY;
  _position;
  ...
  attachOverlay() {
    ...
    if (this.positionChange.observers.length > 0) {
      this._positionSubscription = this._position.positionChanges.pipe((0, import_operators8.takeWhile)(() => this.positionChange.observers.length > 0)).subscribe((position) => {
...
_LightboxComponent.ɵfac = function LightboxComponent_Factory(__ngFactoryType__) {
...
var _Lightbox = class _Lightbox {
  constructor(config, _gallery, _overlay, _sanitizer) {
    ...
    this.opened = new import_rxjs14.Subject();
    this.closed = new import_rxjs14.Subject();
...
_Lightbox.ɵfac = function Lightbox_Factory(__ngFactoryType__) {
...
var _GallerizeDirective = class _GallerizeDirective {
  ...
  detectorMode(galleryRef) {
    this._detector$ = new import_rxjs14.Subject();
    this._detector$.pipe((0, import_rxjs14.debounceTime)(300), (0, import_rxjs14.switchMap)(() => {
      const imageElements = this._el.nativeElement.querySelectorAll(this.selector);
      if (imageElements && imageElements.length) {
        const images = [];
        return (0, import_rxjs14.from)(imageElements).pipe((0, import_rxjs14.map)((el, i) => {
        ...
        }), (0, import_rxjs14.tap)((data) => images.push(new ImageItem(data))), (0, import_rxjs14.finalize)(() => galleryRef.load(images)));
      } else {
...
_GallerizeDirective.ɵfac = function GallerizeDirective_Factory(__ngFactoryType__) {
...
var _LightboxDirective = class _LightboxDirective {
  constructor(_lightbox, _el, _renderer) {
    ...
    this._el = _el;
    this._renderer = _renderer;
    this.clickEvent = import_rxjs14.Subscription.EMPTY;
    this.index = 0;
    this.id = "root";
  }
  ngOnInit() {
    this._renderer.setStyle(this._el.nativeElement, "cursor", "pointer");
    this.clickEvent = (0, import_rxjs14.fromEvent)(this._el.nativeElement, "click").pipe((0, import_rxjs14.tap)(() => this._lightbox.open(this.index, this.id))).subscribe();
  }
  ngOnDestroy() {
    this.clickEvent.unsubscribe();
  }
};
_LightboxDirective.ɵfac = function LightboxDirective_Factory(__ngFactoryType__) {
...
_LightboxModule.ɵfac = function LightboxModule_Factory(__ngFactoryType__) {
